import pandas as pd
import numpy as np

# Example S-box 
sbox = [  
]

# Validation
assert len(sbox) == 256, "S-box must have 256 elements."
assert len(set(sbox)) == 256, "S-box must be a permutation of 0-255."

def generate_ddt(sbox):
    """
    Generate Differential Distribution Table (DDT) for the given S-box
    
    Args:
        sbox (list): S-box lookup table
    
    Returns:
        numpy.ndarray: 256x256 DDT table
    """
    size = len(sbox)
    ddt = np.zeros((size, size), dtype=int)
    
    for input_diff in range(size):
        for u0 in range(size):
            u1 = u0 ^ input_diff
            v0 = sbox[u0]
            v1 = sbox[u1]
            output_diff = v0 ^ v1
            ddt[input_diff][output_diff] += 1
    
    return ddt

# Generate DDT
ddt_table = generate_ddt(sbox)

# Save as CSV without indexing
pd.DataFrame(ddt_table).to_csv("ddt_table.csv", index=False, header=False)

# Optional: print some statistics
print("DDT table shape:", ddt_table.shape)
print("Maximum differential count:", np.max(ddt_table))
print("DDT table saved to 'ddt_table.csv'")

# Optional additional analysis
print("\nTop 5 most frequent differentials:")
sorted_diffs = np.argsort(ddt_table.flatten())[::-1]
top_5_indices = sorted_diffs[:5]
for idx in top_5_indices:
    input_diff = idx // 256
    output_diff = idx % 256
    count = ddt_table[input_diff][output_diff]
    print(f"Input Diff: {input_diff}, Output Diff: {output_diff}, Count: {count}")