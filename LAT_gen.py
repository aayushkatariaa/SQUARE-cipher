import numpy as np
import csv

sbox = [
    
]

def calculate_lat(sbox):
    n = int(len(sbox).bit_length() - 1)  
    lat = np.zeros((1 << n, 1 << n), dtype=int)  

    for input_mask in range(1 << n):  
        for output_mask in range(1 << n):  
            count = 0
            for input_value in range(1 << n):  
                output_value = sbox[input_value]
                input_parity = bin(input_value & input_mask).count('1') % 2  
                output_parity = bin(output_value & output_mask).count('1') % 2  
                if input_parity == output_parity:
                    count += 1
            lat[input_mask, output_mask] = count - (1 << (n - 1))  
    return lat

def save_lat_to_csv(lat, filename='linear_approximation_table.csv'):
    """
    Save the Linear Approximation Table to a CSV file.
    
    Args:
    lat (numpy.ndarray): The Linear Approximation Table
    filename (str): Name of the output CSV file
    """
    with open(filename, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        
        # Write header row (input masks)
        header = ['output_mask\\input_mask'] + [f'{i:03d}' for i in range(lat.shape[1])]
        csvwriter.writerow(header)
        
        # Write each row with its corresponding output mask
        for i, row in enumerate(lat):
            # Prepend the output mask as the first column
            csv_row = [f'{i:03d}'] + list(row)
            csvwriter.writerow(csv_row)
    
    print(f"LAT saved to {filename}")

# Calculate LAT
calculate = calculate_lat(sbox)

# Save to CSV
save_lat_to_csv(calculate)

# Optional: print some basic information about the LAT
print("LAT Shape:", calculate.shape)
print("Maximum absolute value:", np.max(np.abs(calculate)))