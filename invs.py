def compute_inverse_sbox(sbox):
    """
    Compute the inverse of an S-box.
    
    Parameters:
    sbox (list of int): The S-box as a list of 256 integers (0-255).
    
    Returns:
    list of int: The inverse S-box as a list of 256 integers.
    """
    # Ensure the S-box has 256 unique entries
    assert len(sbox) == 256, "S-box must have 256 entries."
    assert len(set(sbox)) == 256, "S-box must have unique entries."
    
    # Initialize the inverse S-box
    inv_sbox = [0] * 256
    
    # Compute the inverse mapping
    for i, value in enumerate(sbox):
        inv_sbox[value] = i
    
    return inv_sbox

# Example S-box
sbox = [
    
]

# Compute the inverse S-box
inv_sbox = compute_inverse_sbox(sbox)

# Print the inverse S-box
print("Inverse S-box:")
for i in range(0, 256, 16):
    print(", ".join(f"0x{value:02X}" for value in inv_sbox[i:i + 16]))
